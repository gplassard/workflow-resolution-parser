...
pub fn parse_index_path(input: &str) -> IResult<&str, PathElement> {
    ...
    let parsed_as_i32: i32 = parsed.parse()
...
mod tests {
    ...
    fn test_parse_attribute_path() {
        ...
        let expected = Ok(("", PathElement::AttributePath {
            name: "foo".to_string()
        }));
    ...
    fn test_parse_attribute_path_stops_at_dot() {
        ...
        let expected = Ok((".bar", PathElement::AttributePath {
            name: "foo".to_string()
        }));
        assert_eq!(result, expected);
    }
    ...
    fn test_parse_index_path() {
        ...
        let expected = Ok(("", PathElement::IndexPath {
            index: 123
        }));
        assert_eq!(result, expected);
    }
    ...
    fn test_parse_index_path_with_remains() {
        ...
        let expected = Ok(("aaa", PathElement::IndexPath {
            index: 123
        }));
        assert_eq!(result, expected);
    }
